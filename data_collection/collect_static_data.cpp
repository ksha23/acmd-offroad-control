...
// most of its samples where the deployed estimator never queries: measured over
// 24k rows of runtime traces, GRIT queries per-wheel loads of 2.9-3.4 kN and
// rear slip angles inside +-0.08 rad, against a box of 2.5-7.5 kN and +-0.6 rad.
// These let a collection concentrate density where the surrogate is actually
// evaluated, without changing the soil ranges the estimator sweeps.
...
struct ParameterRanges {
    // Operating conditions — matches TRAINING_RANGES in
    // simulation/shared/param_consistency.py, so the sampled corpus and the
    // runtime feature ranges cannot drift apart.
    ...
    // Terrain — matches the same TRAINING_RANGES soil bounds
    ...
    double mohr_friction_min = 6.0, mohr_friction_max = 37.8;   // degrees (0.105-0.66 rad)
...
struct StaticResult {
    ...
    double My_avg = 0;             // spin-axis SCM contact moment at spindle (N m)
    double sinkage_avg = 0;        // penetration below the initial rig surface (m)
...
static void ApplyOverrides(ParameterRanges& r) {
    ...
    if (g_overrides.alpha_min < 900) r.slip_angle_min = g_overrides.alpha_min;
    if (g_overrides.alpha_max < 900) r.slip_angle_max = g_overrides.alpha_max;
    if (g_overrides.u_min > 0) r.velocity_min = g_overrides.u_min;
    if (g_overrides.u_max > 0) r.velocity_max = g_overrides.u_max;
    if (g_overrides.kappa_min < 900) r.slip_ratio_min = g_overrides.kappa_min;
    if (g_overrides.kappa_max < 900) r.slip_ratio_max = g_overrides.kappa_max;
}
...
std::vector<StaticSampleParams> GenerateFactoredSamples(int n, const ParameterRanges& r,
                                                        int terrain_bank_size,
                                                        unsigned int seed) {
    std::mt19937 rng(seed);
...
StaticResult CollectStaticSample(const StaticSampleParams& params, int sample_id, bool visualize = false) {
    ...
    try {
        ...
        // Capture the undeformed surface before advancing the wheel.  For an
        // SCM rigid tire, ReportTireForce().point is the spindle centre, so
        // surface_z - (spindle_z - radius) is a direct controlled-rig sinkage
        // label without reading individual terrain nodes.
                                   ...
                                   int shard_id, int num_shards)
{
    std::cout << "\n=== Subprocess-Batched Static Data Collection ===\n"
              ...
              << "Mesh spacing: "
              << (fixed_mesh_spacing > 0.0 ? std::to_string(fixed_mesh_spacing)
                                           : std::string("LHS [0.08, 0.12]"))
              << " m\n"
    ...
    if (fixed_mesh_spacing > 0.0) {
        ranges.mesh_spacing_min = fixed_mesh_spacing;
        ranges.mesh_spacing_max = fixed_mesh_spacing;
    }
    auto all_samples = use_factored_sampling
        ? GenerateFactoredSamples(n_samples, ranges, terrain_bank_size, random_seed)
        : GenerateLHSSamples(n_samples, ranges, random_seed);
...
void CollectStaticData(int n_samples, const std::string& output_file,
                       ...
                       bool use_factored_sampling, int terrain_bank_size,
                       unsigned int random_seed, double fixed_mesh_spacing)
{
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--threads" || arg == "-t") {
            ...
            if (i + 1 < argc) fixed_mesh_spacing = std::stod(argv[++i]);
        } else if (arg == "--fz-min") {
            ...
            std::cout << "Usage: " << argv[0] << " [num_samples] [output.csv] [options]\n\n"
                      ...
                      << "  --seed N             Deterministic LHS seed (default: 42)\n"
                      << "  --mesh-spacing M     Fix SCM grid spacing instead of sampling it\n"
    ...
    try {
        if (fixed_mesh_spacing < 0.0) {
            throw std::invalid_argument("--mesh-spacing must be positive");
        }
        ...
        if (batch_size > 0) {
            CollectWithSubprocessBatching(
                ...
                use_factored_sampling, terrain_bank_size, random_seed,
                fixed_mesh_spacing, shard_id, num_shards
            );
            return 0;
        }
#endif
