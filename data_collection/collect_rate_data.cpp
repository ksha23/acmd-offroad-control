...
// Operating-point overrides (CLI). Uniform LHS over the full envelope spends
// most samples where the deployed estimator never queries. ChTireTestRig cannot
// realise a prescribed load below 3*(wheel+tire)*g, about 1660 N for this
// wheel: below that it prints "Prescribed normal load too small" and silently
// keeps the default chassis mass, so the row carries a load never applied.
struct RangeOverrides {
    double fz_min = -1, fz_max = -1;
    double alpha_min = 999, alpha_max = 999;
    double u_min = -1, u_max = -1;
    double kappa_min = 999, kappa_max = 999;
    // Terrain pins, for controlled single-factor sweeps: setting min == max
    // collapses that LHS coordinate to a constant so only the swept factor moves.
    double n_min = -1, n_max = -1;
    double kphi_min = -1, kphi_max = -1;
    double kc_min = -1, kc_max = -1;
    double cohesion_min = -1, cohesion_max = -1;
    double friction_min = -1, friction_max = -1;
    double janosi_min = -1, janosi_max = -1;
    double mesh_min = -1, mesh_max = -1;
    double dkappa_min = 999, dkappa_max = 999;
    double dalpha_min = 999, dalpha_max = 999;
    double dv_min = 999, dv_max = 999;
};
static RangeOverrides g_overrides;
static void ApplyOverrides(ParameterRanges& r) {
    if (g_overrides.fz_min > 0) r.vertical_load_min = g_overrides.fz_min;
    if (g_overrides.fz_max > 0) r.vertical_load_max = g_overrides.fz_max;
    if (g_overrides.alpha_min < 900) r.alpha_target_min = g_overrides.alpha_min;
    if (g_overrides.alpha_max < 900) r.alpha_target_max = g_overrides.alpha_max;
    if (g_overrides.u_min > 0) r.velocity_target_min = g_overrides.u_min;
    if (g_overrides.u_max > 0) r.velocity_target_max = g_overrides.u_max;
    if (g_overrides.kappa_min < 900) r.kappa_target_min = g_overrides.kappa_min;
    if (g_overrides.kappa_max < 900) r.kappa_target_max = g_overrides.kappa_max;
    if (g_overrides.n_min > 0) r.bekker_n_min = g_overrides.n_min;
    if (g_overrides.n_max > 0) r.bekker_n_max = g_overrides.n_max;
    if (g_overrides.kphi_min > 0) r.bekker_Kphi_min = g_overrides.kphi_min;
    if (g_overrides.kphi_max > 0) r.bekker_Kphi_max = g_overrides.kphi_max;
    if (g_overrides.kc_min >= 0) r.bekker_Kc_min = g_overrides.kc_min;
    if (g_overrides.kc_max >= 0) r.bekker_Kc_max = g_overrides.kc_max;
    if (g_overrides.cohesion_min > 0) r.mohr_cohesion_min = g_overrides.cohesion_min;
    if (g_overrides.cohesion_max > 0) r.mohr_cohesion_max = g_overrides.cohesion_max;
    if (g_overrides.friction_min > 0) r.mohr_friction_min = g_overrides.friction_min;
    if (g_overrides.friction_max > 0) r.mohr_friction_max = g_overrides.friction_max;
    if (g_overrides.janosi_min > 0) r.janosi_shear_min = g_overrides.janosi_min;
    if (g_overrides.janosi_max > 0) r.janosi_shear_max = g_overrides.janosi_max;
    if (g_overrides.mesh_min > 0) r.mesh_spacing_min = g_overrides.mesh_min;
    if (g_overrides.mesh_max > 0) r.mesh_spacing_max = g_overrides.mesh_max;
    if (g_overrides.dkappa_min < 900) r.dkappa_min = g_overrides.dkappa_min;
    if (g_overrides.dkappa_max < 900) r.dkappa_max = g_overrides.dkappa_max;
    if (g_overrides.dalpha_min < 900) r.dalpha_min = g_overrides.dalpha_min;
    if (g_overrides.dalpha_max < 900) r.dalpha_max = g_overrides.dalpha_max;
    if (g_overrides.dv_min < 900) r.dvelocity_min = g_overrides.dv_min;
    if (g_overrides.dv_max < 900) r.dvelocity_max = g_overrides.dv_max;
}
...
struct RateResult {
    ...
    double My_avg;          // spin-axis SCM contact moment at spindle (N m)
    double sinkage_avg;     // penetration below the initial rig surface (m)
    int n_samples;
    ...
    const RateSampleParams& params, int sample_id, bool visualize = false)
{
    ...
    try {
        ...
        const auto initial_spindle = rig.ReportTireForce().point;
        const double initial_surface_z = rig.GetTerrain()->GetHeight(initial_spindle);
        ...
        double sum_Fx = 0, sum_Fy = 0, sum_Fz = 0, sum_My = 0, sum_sinkage = 0;
        ...
        while (t < t_end) {
            ...
            if (t >= t_next_measure && t >= t_measure_start && t <= t_measure_end) {
                ...
                sum_My += force.moment.y();
                sum_sinkage += initial_surface_z - (force.point.z() - tire->GetRadius());
                auto spindle = tire->GetWheel()->GetSpindle();
                auto spindle_vel = spindle->GetPosDt();
                double cur_kappa = tire->GetLongitudinalSlip();
                double cur_alpha = tire->GetSlipAngle();
                double cur_v = std::hypot(spindle_vel.x(), spindle_vel.y());
                sum_kappa += cur_kappa;
                sum_alpha += cur_alpha;
                sum_v     += cur_v;
                sum_omega += spindle->GetAngVelLocal().y();
        ...
        if (count > 0) {
            ...
            result.My_avg = sum_My / count;
            result.sinkage_avg = sum_sinkage / count;
...
static const std::string CSV_HEADER =
    ...
    "mohr_friction,janosi_shear,mesh_spacing,Fz,Fx,Fy,My,sinkage,"
    "kappa_cmd,alpha_cmd,velocity_cmd\n";
                     ...
                     const RateResult& result)
{
    ...
    if (!finite(result.actual_kappa) || !finite(result.actual_alpha) ||
        ...
        !finite(result.My_avg) || !finite(result.sinkage_avg)) {
        return;
    }
    ...
    buf << result.actual_kappa << ","
        ...
        << result.My_avg << ","
        << result.sinkage_avg << ","
        // Commanded operating point at the measurement instant. The rig's own
        // accessors for this were removed from Chrono, but the collector builds
        // the ramp itself: kappa(t)=a*t+b*t^2 reaches the target exactly at t=T.
        // Recorded alongside the tire-computed values so a surrogate can be
        // conditioned on whichever the runtime actually supplies.
        << params.kappa_target << ","
        << params.alpha_target << ","
        << params.velocity_target << "\n";
...
void CollectWithSubprocessBatching(int n_samples, const std::string& output_file,
                                   ...
                                   int shard_id, int num_shards, unsigned int random_seed)
{
    ...
    ApplyOverrides(ranges);
    auto all_samples = GenerateLHSSamples(n_samples, ranges, random_seed);

    std::signal(SIGINT, signal_handler);
    std::signal(SIGTERM, signal_handler);

    // Shards split one shared LHS design by contiguous slice, so the union over
    // shards is exactly the single-process design and sample ids stay unique.
    if (num_shards < 1) num_shards = 1;
    if (shard_id < 0 || shard_id >= num_shards) {
        std::cerr << "--shard must be in [0, --num-shards)\n";
        return;
    }
    const int shard_lo = static_cast<int>((long long)n_samples * shard_id / num_shards);
    const int shard_hi = static_cast<int>((long long)n_samples * (shard_id + 1) / num_shards);
    const int shard_n = shard_hi - shard_lo;
    if (num_shards > 1) {
        std::cout << "Shard " << shard_id << "/" << num_shards << ": samples "
                  << shard_lo << "-" << (shard_hi - 1) << " (" << shard_n << ")\n";
    }

    auto t0 = std::chrono::high_resolution_clock::now();
    int total_ok = 0, total_done = 0;

    for (int bs = shard_lo; bs < shard_hi && !g_stop; bs += batch_size) {
        int be = std::min(bs + batch_size, shard_hi);
        int cur = be - bs;
...
void CollectRateData(int n_samples, const std::string& output_file,
                     int num_threads, bool use_parallel, bool visualize, unsigned int random_seed)
{
    ...
    if (continuous) {
        ...
        auto samples = GenerateLHSSamples(n_samples, ranges, random_seed);
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--threads" || arg == "-t") {
            ...
            if (i + 1 < argc) g_overrides.n_min = std::stod(argv[++i]);
        } else if (arg == "--n-max") {
            if (i + 1 < argc) g_overrides.n_max = std::stod(argv[++i]);
        } else if (arg == "--kphi-min") {
            if (i + 1 < argc) g_overrides.kphi_min = std::stod(argv[++i]);
        } else if (arg == "--kphi-max") {
            if (i + 1 < argc) g_overrides.kphi_max = std::stod(argv[++i]);
        } else if (arg == "--kc-min") {
            if (i + 1 < argc) g_overrides.kc_min = std::stod(argv[++i]);
        } else if (arg == "--kc-max") {
            if (i + 1 < argc) g_overrides.kc_max = std::stod(argv[++i]);
        } else if (arg == "--cohesion-min") {
            if (i + 1 < argc) g_overrides.cohesion_min = std::stod(argv[++i]);
        } else if (arg == "--cohesion-max") {
            if (i + 1 < argc) g_overrides.cohesion_max = std::stod(argv[++i]);
        } else if (arg == "--friction-min") {
            if (i + 1 < argc) g_overrides.friction_min = std::stod(argv[++i]);
        } else if (arg == "--friction-max") {
            if (i + 1 < argc) g_overrides.friction_max = std::stod(argv[++i]);
        } else if (arg == "--janosi-min") {
            if (i + 1 < argc) g_overrides.janosi_min = std::stod(argv[++i]);
        } else if (arg == "--janosi-max") {
            if (i + 1 < argc) g_overrides.janosi_max = std::stod(argv[++i]);
        } else if (arg == "--dkappa-min") {
            if (i + 1 < argc) g_overrides.dkappa_min = std::stod(argv[++i]);
        } else if (arg == "--dkappa-max") {
            if (i + 1 < argc) g_overrides.dkappa_max = std::stod(argv[++i]);
        } else if (arg == "--dalpha-min") {
            if (i + 1 < argc) g_overrides.dalpha_min = std::stod(argv[++i]);
        } else if (arg == "--dalpha-max") {
            if (i + 1 < argc) g_overrides.dalpha_max = std::stod(argv[++i]);
        } else if (arg == "--dv-min") {
            if (i + 1 < argc) g_overrides.dv_min = std::stod(argv[++i]);
        } else if (arg == "--dv-max") {
            if (i + 1 < argc) g_overrides.dv_max = std::stod(argv[++i]);
        } else if (arg == "--mesh-spacing") {
            if (i + 1 < argc) { g_overrides.mesh_min = g_overrides.mesh_max = std::stod(argv[++i]); }
        } else if (arg == "--seed") {
            if (i + 1 < argc) random_seed = static_cast<unsigned int>(std::stoul(argv[++i]));
        } else if (arg == "--shard") {
            if (i + 1 < argc) shard_id = std::atoi(argv[++i]);
        } else if (arg == "--num-shards") {
            if (i + 1 < argc) num_shards = std::atoi(argv[++i]);
            ...
            if (i + 1 < argc) g_overrides.fz_min = std::stod(argv[++i]);
        } else if (arg == "--fz-max") {
            if (i + 1 < argc) g_overrides.fz_max = std::stod(argv[++i]);
        } else if (arg == "--alpha-min") {
            if (i + 1 < argc) g_overrides.alpha_min = std::stod(argv[++i]);
        } else if (arg == "--alpha-max") {
            if (i + 1 < argc) g_overrides.alpha_max = std::stod(argv[++i]);
        } else if (arg == "--u-min") {
            if (i + 1 < argc) g_overrides.u_min = std::stod(argv[++i]);
        } else if (arg == "--u-max") {
            if (i + 1 < argc) g_overrides.u_max = std::stod(argv[++i]);
        } else if (arg == "--kappa-min") {
            if (i + 1 < argc) g_overrides.kappa_min = std::stod(argv[++i]);
        } else if (arg == "--kappa-max") {
            if (i + 1 < argc) g_overrides.kappa_max = std::stod(argv[++i]);
    ...
    try {
        ...
        if (batch_size > 0 && n_samples > 0) {
            CollectWithSubprocessBatching(n_samples, output_file, num_threads, use_parallel,
                                         batch_size, shard_id, num_shards, random_seed);
            return 0;
        }
#endif

        CollectRateData(n_samples, output_file, num_threads, use_parallel, visualize, random_seed);
    } catch (const std::exception& e) {
