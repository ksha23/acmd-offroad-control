...
Generates the procedural heightmap that gives the SCM patch its topology,
separately from its soil parameters. Slope limiting bounds the steepest grade
the generated surface may present, so a bumpiness level stays traversable and
the topology is an independent factor in the terrain study rather than a source
of unrecoverable states. A seed makes any heightmap reproducible.
"""
...
class PerlinNoise:
    ...
    Seeded from a permutation shuffle, so a given seed always yields the same
    field.
    """
...
def generate_heightmap_bmp(filename: str, width: int, height: int,
    ...
    with open(filename, 'wb') as f:
        ...
        # Second pass: limit the slope, so no generated grade exceeds
        # max_slope. The pixel-space bound is max_slope times the metres per
        # pixel; at two pixels per metre the spacing is 0.5 m.
        ...
        # Iterative relaxation toward the slope bound, in the manner of an
        # erosion pass: each sweep pulls a cell toward its neighbours wherever
        # the local grade exceeds the limit.
