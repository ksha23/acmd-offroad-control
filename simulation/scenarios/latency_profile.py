#!/usr/bin/env python3
"""JSON-configurable latency profiles for teleoperation studies.

A profile supplies the one-way delay on each channel -- command, operator
input, and camera -- as a function of simulated time, so a run faces a
time-varying link rather than a single fixed delay. Two sources are supported:
replay of a recorded bitrate trace mapped through a queueing model, and a
seeded statistical model combining a lognormal baseline with Poisson
handover spikes. Neither loads a learned sequence model at runtime, so the
latency environment introduces no trained component into the evaluation.
"""
...
class LatencyProfile:
    ...
    def delay(self, sim_time_s: float, channel: str = "control") -> float:
        # phase_s selects which window of the looped trace a run replays.
        # Without it every short run replays the head of the trace, which in a
        # segment-scheduled profile is a nominal stretch containing no outage
        # or burst, so the battery would never sample the hard regimes.
        # Staggered phases let a set of short runs cover the whole environment
        # collectively, while paired arms share a phase and therefore face
        # identical delays.
        t = sim_time_s + getattr(self, "phase_s", 0.0)
        if channel in self.channels:
            return self.channels[channel].delay(t)
        if "control" in self.channels:
            return self.channels["control"].delay(t)
        first = next(iter(self.channels.values()))
        return first.delay(t)
...
def _lognormal_spikes_trace(cfg: dict[str, Any], n: int, sample_period_s: float,
    ...
    """Statistical latency model, independent of any traffic trace.

    A right-skewed lognormal baseline representing ordinary 5G delay
    variability, plus brief Poisson-arrival handover and congestion spikes.
    The parameters are calibrated against representative published 5G
    remote-driving latency figures; the result is a model of that environment
    rather than a measurement of one. Returns delay in seconds.
    """
...
def _load_or_generate_bitrate(
    ...
    if source.get("kind") == "5g_repo_csv":
        ...
        # A profile naming a 5G traffic CSV requires that file. Falling back to
        # the synthetic load would saturate the queueing model and produce a
        # degenerate near-constant latency, silently replacing the measured
        # environment with one that exercises none of its variation. Failing
        # here makes the substitution impossible.
        if not raw_path.exists():
            raise FileNotFoundError(
                f"[LATENCY] profile '{cfg_path.name}' names a 5G traffic CSV "
                f"that is missing:\n    {raw_path}\nRestore or regenerate it "
                f"from the generated 5G data set under data/5g_generated. "
                f"Falling back to a synthetic load is refused: it degenerates "
                f"to a near-constant-latency profile.")
