...
Loads the reference paths a run tracks and checks that a requested path is
within the HMMWV's turning capability, so an infeasible geometry is rejected at
configuration time rather than appearing as a tracking failure in the results.
"""
...
def find_closest_point_on_sinusoid(x_veh, y_veh, amplitude, wavelength, search_range=10.0, n_samples=100,
    ...
    # Refine the sampled minimum with damped Newton steps on the squared
    # distance, so the projection is not quantized to the sample spacing.
...
def suggest_feasible_sine_params(target_amplitude=2.0, wheelbase=3.302, delta_max=0.5, margin=1.2):
    """
    Return the shortest wavelength that keeps a sinusoid feasible at a given
    amplitude.

    Args:
        target_amplitude: Desired amplitude (m)
        margin: Multiplier on the minimum turning radius, leaving room for
            tracking error; 1.2 requires 20 percent more radius than the
            kinematic limit.
...
def make_path_function(path_type='lane_change', v_target=8.0,
    ...
    """
    Load a ReferencePath from the waypoint CSV ``data/paths/<path_type>.csv``.

    The CSV must contain ``x,y`` columns, or the four-column ``s,x,y,psi``
    form. If *lead_in* > 0 a straight section is prepended along +x, giving the
    vehicle room to reach the target speed before the path geometry begins.

    Args:
        path_type: Name of the path, matching ``data/paths/<name>.csv``.
