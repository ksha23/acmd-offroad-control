...
The path is fixed for the duration of a run and is identical across every arm
of a comparison, so any difference in tracking error is attributable to the
controller and terrain rather than to the reference.

Workflow
--------
1.  At startup, generate dense waypoints for the chosen path type, or load them
    from a saved CSV.
...
class ReferencePath:
    ...
    def __init__(self, x_pts, y_pts, v_target, blend_fraction=0.5,
        ...
        blend_fraction : float
            Fraction of the MPC horizon (0-1) over which the reference blends
            from the vehicle's current state to the desired path. At 0 the
            whole reference lies on the desired path, which asks a vehicle that
            has drifted off-path to rejoin it within one step; the default of
            0.5 blends over half the horizon.
        ...
        # Fraction of the physical Coulomb lateral-acceleration limit the speed
        # profiler may use in turns. The default of 0.65 leaves margin for the
        # cohesion-derived traction SCM soils provide above the Coulomb floor;
        # raising it makes the profile request faster cornering.
        ...
        # A direct x-to-y spline for x-based reference generation. Sampling by
        # arc length through a lane change advances less far in x than the same
        # distance on a straight, which shortens the effective horizon exactly
        # where the manoeuvre demands it. Valid only where x is strictly
        # monotonic.
        ...
        # Pre-compute a curvature-aware speed profile over the entire path, so
        # the vehicle can brake for turns lying beyond the MPC horizon. At
        # higher speeds the two-second lookahead is shorter than the braking
        # distance, and a horizon-limited profile would enter such a turn too
        # fast to correct.
    ...
    def _build_speed_profile(self):
        ...
        """
        # Profile limits for SCM terrain.
        ay_comfort = 3.5   # m/s^2 — lateral acceleration budget in turns
        a_brake = 2.0      # m/s^2 — inside the NMPC's ax_min of -2.6
        a_accel = 3.0      # m/s^2 — acceleration out of turns

        # Terrain-aware ay_comfort. On a low-friction surface -- clay at a
        # friction angle of 13 degrees gives mu of about 0.23 -- the Coulomb
        # lateral-acceleration limit mu*g is roughly 2.26 m/s^2. SCM soils
        # supply cohesion-based traction above that floor, with cohesion and
        # Janosi shear contributing a substantial share of the slip-angle
        # traction, so the budget is set as a fraction of the Coulomb limit
    ...
    def curvature_at(self, s):
        ...
        kappa = |x'(s) y''(s) - y'(s) x''(s)| / (x'^2 + y'^2)^1.5.

        Evaluating the analytic derivative makes the result independent of
        query spacing. Finite-differencing the MPC horizon instead would make
        curvature depend on speed: the horizon collapses to a few metres at low
        or varying speed, producing spurious curvature spikes that in turn
        depress the reference speed without cause.
        """
    ...
    def get_reference(self, time, z0, N, dt):
        ...
        # 1. Forward-constrained closest-waypoint search. The search starts at
        #    _last_idx, with a small backward window absorbing measurement
        #    noise, so the progress index cannot jump back to an earlier
        #    section of a symmetric or sinusoidal path that happens to lie
        #    equally close.
        search_back = 10   # waypoints of permitted backward look
    ...
    def closest_point_on_path(self, x_q: float, y_q: float) -> dict:
        ...
        This is valid for every path shape. Taking ``evaluate_at_x`` and the
        difference ``y - y_ref`` is a lateral proxy only where ``x`` is a good
        progress variable, and it misreports the error on a general curve or
        wherever the vehicle is barely moving.
        """
        xc = self.cs_x(self.s)
        yc = self.cs_y(self.s)
        dxw = xc - x_q
    ...
    def save_csv(self, filepath):
        """Write waypoints and the precomputed speed profile to CSV."""
        ...
        with open(filepath, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['s', 'x', 'y', 'psi', 'v_profile_mps'])
