...
class SpatialTransitionSpec:
    ...
    def __post_init__(self):
        for name in (self.start_preset, self.end_preset):
            if name not in TERRAIN_PRESETS and not _is_soil_yaml(name):
                raise ValueError(
                    f"Unknown terrain preset: {name!r} (not a preset name and "
                    f"not a soil .yaml path). "
                    f"Available presets: {list(TERRAIN_PRESETS.keys())}"
                )
        if self.transition_width < 0:
            raise ValueError("transition_width must be >= 0")


def _is_soil_yaml(name: str) -> bool:
    import os
    return (isinstance(name, str) and name.endswith((".yaml", ".yml"))
            and os.path.isfile(name))


def _preset_vec(name: str) -> dict[str, float]:
    """The eight soil scalars for a preset name OR a custom soil .yaml path,
    filling Chrono defaults if absent. Custom yamls let benchmarks build
    transitions between arbitrary (e.g. off-manifold) soils."""
    if _is_soil_yaml(name):
        import yaml as _yaml
        with open(name) as fh:
            preset = _yaml.safe_load(fh)
    else:
