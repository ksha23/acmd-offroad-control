...
def _set_always_on_top(pg):
    """Best-effort: pin above + raise WITHOUT taking focus (X11).

    Called periodically, not just at startup: the fullscreen sensor window
    maps after the HUD and restacks over a once-pinned overlay on most
    window managers, so the pin must be re-asserted while running.
    xdotool windowraise raises the stacking order without stealing focus
    from the driving window."""
    try:
        ...
        if shutil.which("xdotool"):
            subprocess.run(["xdotool", "windowraise", str(int(wid))],
                           check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception:
        pass
...
def main():
    ...
    while running:
        ...
        if not args.smoke and (frames == 1 or frames % 60 == 0):
            _set_always_on_top(pg)   # re-assert: the sim window maps later
