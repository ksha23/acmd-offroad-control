...
stress integration) rather than the tire-rig surrogate. It remains wired as an
analytical comparison backend via ``--terrain-estimator-backend bekker_ukf``.
...
tick), reconstructing the steering angle from the slip angle + state. Because
the soil is not known online, the five non-n Bekker--Mohr params
are taken from the canonical clay->dirt->sand manifold evaluated at the current
n estimate (matching the deployed rig-UKF); only n is the
...
from terrain_parameterization import (  # noqa: E402
    N_BOUNDS as _N_BOUNDS,
    terrain_params_for_n as _terrain_params_for_n,
)
