...
This calibration supplies the static drag feedforward that the ``ffdrag`` arm of
``ff_drag_ablation.py`` evaluates, and quantifies the longitudinal prediction
gap the manuscript localizes in Sec. 3.1 when it reports that heading prediction
is accurate over the horizon while speed prediction drifts.

The NMPC longitudinal channel is kinematic, u_dot = ax + du_dot_resid, and with
du_dot_resid at zero it carries no compaction resistance, so on deformable soil
it predicts more speed than the plant delivers. The missing term is recovered
from the logged rollouts rather than assumed: at each solve the controller logs
its predicted speed u_pred(tau), and pairing that against the realized u(t+tau)
gives a signed drift Delta u(tau) = u_pred - u_act. Because the drift
accumulates at the rate of the unmodelled deceleration, its slope over the
horizon is c_drag in m/s^2. The slope is fit per terrain and indexed by the
sinkage exponent n, which lets the controller look the term up from its live
estimate n_hat.

The logs are collected with the throttle disturbance observer disabled by
``collect_drag_calib_logs.py``, so the observer's own correction cannot mask the
