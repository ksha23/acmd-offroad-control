#!/usr/bin/env python3
"""Matrix study of obstacle avoidance across the planner and the command filter.

This study produces the four matrix rows of Table 3 in the manuscript: mean
native contacts per run, mean signed clearance, filter intervention share, and
mean absolute steering correction, for each combination of command filter (none
or DOB-CBF) and planner obstacle awareness, over the full terrain, path, speed,
bumpiness, and seed matrix.

Two factors are crossed. The command filter is either absent or the reactive
DOB-CBF backstop. The planner is either given the obstacle coordinates, so the
NMPC's in-horizon soft penalty can avoid them, or denied them with
``--mpc-blind-obstacles``, which leaves the filter as the sole avoider. Crossing
the two separates the contribution of the in-horizon penalty from that of the
backstop and shows what the two achieve only in combination, which is the
composition claim Sec. 4.1 makes.

Collisions are native Chrono body contacts between an ego body and an obstacle
body. Signed clearance is a geometric diagnostic and never establishes a
collision. Sensor noise is enabled in every run.
"""
...
from paper_provenance import acquire_paper_ros_lease  # noqa: E402


FLAVORS = ("none", "dob_cbf", "mpsf")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--blind-and-aware", action="store_true",
                   help="Cross each filter with both planner-aware and "
                        "planner-blind obstacle cases, which is the four-variant "
                        "design the manuscript reports.")
    p.add_argument("--shield-horizon", type=int, default=18)
    p.add_argument("--safety-buffer", type=float, default=0.50,
                   help="Extra obstacle clearance buffer passed to the DOB-CBF "
                        "filter. Default 0.50 m gives it a practical guard band "
                        "beyond the geometric vehicle-clearance proxy.")
...
def main() -> None:
    ...
    # differ, and the cross-talk would silently mix runs. Hold the lease for the
    # whole matrix; releasing it early would reopen the window it closes.
    lease = acquire_paper_ros_lease("safety_filter_sweep")
    try:
        _run(args)
    finally:
        lease.close()
...
def _run(args: argparse.Namespace) -> None:
    ...
    save_summary_markdown(
        ...
        [
            ...
            f"Safety buffer: {args.safety_buffer:g} m beyond the geometric "
            "vehicle-clearance proxy; collision truth is Chrono body contact.",
            "Residual collisions are diagnosed from the per-run "
            "`cbf_filter_log.csv` and trajectory, which distinguish a "
            "short-horizon braking response from insufficient steering "
            "commitment: DOB-CBF carries an explicit reactive steering layer "
            "and can clear a head-on obstacle without long-horizon optimization.",
        ],
    )
