#!/usr/bin/env python3
"""Train an N-HiTS 5G traffic generator and export a runtime latency profile.

This script regenerates the transport-delay environment the manuscript describes
in Sec. 4.2 and applies throughout the teleoperation studies of Sec. 4. It wraps
the public 5G-Traffic-Generator project, whose neural hierarchical-interpolation
model is trained on packet captures from a commercial 5G network, and exports
its generated traffic together with a latency-profile JSON the simulator
consumes directly.

Traffic is mapped to one-way delay through a queue-load link model: the uplink
series drives the operator command channel and a scaled downlink series drives
the camera channel, giving the asymmetric link the manuscript reports. The
segment schedule places good, poor, and outage regimes at fixed times, so a
replayed run encounters the same conditions whenever it enters the trace at the
same phase.

This generator sits outside the control and estimation stack: it is trained on
network measurements alone and consumes no vehicle or terrain quantity, so it
introduces no learned component into the vehicle pipeline.
"""
...
def patch_repo(repo_dir: Path) -> None:
    """Apply the compatibility edits the upstream project needs to run here.

    Three edits are required. The first passes the drop axis to pandas by
    keyword, which newer pandas requires. The second keys the hyperparameter and
    checkpoint paths on the experiment identifier rather than the data type, so
    that uplink and downlink models trained under different identifiers do not
    overwrite one another. The third restarts the data loader on exhaustion, so
    that an inference length exceeding the loader's size continues to produce
    samples rather than raising.
    """
...
def export_outputs(args: argparse.Namespace, repo_dir: Path) -> Path:
    ...
    profile_dir = ROOT / "data" / "latency_profiles"
