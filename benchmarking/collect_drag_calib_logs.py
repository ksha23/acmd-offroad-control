#!/usr/bin/env python3
"""Collect NMPC prediction logs for motion-resistance calibration.

This script produces the input to ``calibrate_motion_resistance.py`` and to the
open-loop rollout-drift check the manuscript reports in Sec. 3.1, where NMPC
predictions are compared against the Chrono plant over the horizon.

The longitudinal channel is u_dot = ax + du_dot_resid with du_dot_resid at zero,
so disabling the throttle disturbance observer as well leaves the difference
between predicted and realized speed attributable to compaction resistance
alone: with the observer active, its throttle correction would absorb part of
the very drift being measured. Predictions are logged through
``LOG_MPC_PREDICTIONS`` over clay, dirt, and sand at several target speeds and
seeds, each run with the soil known and no online estimator, so the resulting
calibration can be indexed by the sinkage exponent without estimation error
entering it.
"""
...
def _run_one(task: _Task):
    os.environ["LOG_MPC_PREDICTIONS"] = "1"   # enable the controller's prediction dump
    ...
    for terr in TERRAINS:
        for sp in args.speeds:
            for si in range(args.seeds):
                ...
                idx += 1
    print(f"{len(tasks)} runs with the throttle observer disabled, logging predictions")
    r0 = _run_one(tasks[0])
    print("  warmup:", r0)
